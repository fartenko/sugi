...
int8_t sugi_gfx_pset(int32_t x, int32_t y, uint8_t c_in)
{
    ...
    int32_t sugi_gfx_camera_x = 
        *(sugi_memory_ptr + SUGI_MEM_CAMERA_X_PTR + 0) |
    ...
    int32_t sugi_gfx_camera_y = 
        *(sugi_memory_ptr + SUGI_MEM_CAMERA_Y_PTR + 0) |
    ...
    uint16_t sugi_gfx_clip_x1 = 
        *(sugi_memory_ptr + SUGI_MEM_CLIP_LOW_PTR + 0) | 
        *(sugi_memory_ptr + SUGI_MEM_CLIP_HIGH_PTR + 0) << 8;
    uint16_t sugi_gfx_clip_y1 = 
        *(sugi_memory_ptr + SUGI_MEM_CLIP_LOW_PTR + 1) | 
        *(sugi_memory_ptr + SUGI_MEM_CLIP_HIGH_PTR + 1) << 8;
    uint16_t sugi_gfx_clip_x2 = 
        *(sugi_memory_ptr + SUGI_MEM_CLIP_LOW_PTR + 2) | 
        *(sugi_memory_ptr + SUGI_MEM_CLIP_HIGH_PTR + 2) << 8;
    uint16_t sugi_gfx_clip_y2 = 
        *(sugi_memory_ptr + SUGI_MEM_CLIP_LOW_PTR + 3) | 
        *(sugi_memory_ptr + SUGI_MEM_CLIP_HIGH_PTR + 3) << 8;
...
void sugi_gfx_print(char *str, int32_t x, int32_t y, uint8_t c)
{
    ...
    for (int i = 0; i < slen; i++)
    {
        ...
        // TODO: Instead of inerpreting TAB symbol, replace it with SUGI_FONT_TAB_SIZE spaces
        // Tab case
        // if (ascii == 9){
        ...
        // NL case
        if (ascii == 10){
            oy += SUGI_FONT_HEIGHT + SUGI_FONT_LINE_SPACE;
            ox = orig_ox;
            continue;
        }
        // CR case
        ...
        for (int y = 0; y < SUGI_FONT_HEIGHT; y++)
        {
            ...
            {
                if (c & 0x01 != 0)
                    sugi_gfx_pset_no_col(ox - x + SUGI_FONT_WIDTH - 1, oy + y + SUGI_FONT_HEIGHT_OFFSET);
