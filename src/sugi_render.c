...
void sugi_renderer_draw_internal(void)
{
    ...
    // writing zoom and offset to buffers
    sugi_window_zoom     = zoom;
    sugi_window_offset_x = (sw - rw * zoom) / 2;
    sugi_window_offset_y = (sh - rh * zoom) / 2;
    ...
    sugi_renderer_gl_set_viewport_internal(rw * zoom, rh * zoom, sugi_window_offset_x, sugi_window_offset_y);  // (sw - rw * zoom) / 2, (sh - rh * zoom) / 2);
